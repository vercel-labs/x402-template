// forked from https://github.com/modelcontextprotocol/typescript-sdk/blob/79d11dc53fda1dffcbfdad101d9832227134bf7c/src/client/streamableHttp.ts
import {
  Transport,
  FetchLike,
...
export type StreamableHTTPClientTransportOptions = {
   ...
   */
  x402Configuration?: X402Configuration;
};

export interface MessageExtraInfoWithPayment extends MessageExtraInfo {
  paymentInfo?: ReturnType<typeof decodeXPaymentResponse>;
}
...
export class StreamableHTTPClientTransport implements Transport {
  ...
  private _x402Configuration?: Required<X402Configuration>;

  onclose?: () => void;
  onerror?: (error: Error) => void;
  onmessage?: (
    message: JSONRPCMessage,
    extra?: MessageExtraInfoWithPayment
  ) => void;
  ...
  constructor(url: URL, opts?: StreamableHTTPClientTransportOptions) {
    ...
    this._x402Configuration = opts?.x402Configuration && {
      walletClient: opts.x402Configuration.walletClient,
      maxPaymentValue:
        opts.x402Configuration.maxPaymentValue ?? BigInt(0.1 * 10 ** 6), // 0.10 USDC
      paymentRequirementsSelector:
        opts.x402Configuration.paymentRequirementsSelector ??
        selectPaymentRequirements,
    };
  }
  ...
  private async _startOrAuthSse(options: StartSSEOptions): Promise<void> {
    ...
    try {
      ...
      this._handleSseStream(response.body, response.headers, options, true);
    } catch (error) {
      this.onerror?.(error as Error);
      throw error;
    }
        ...
        while (true) {
          ...
          if (!event.event || event.event === "message") {
            try {
              ...
              const paymentInfo = headers.get("X-PAYMENT-RESPONSE")
                ? decodeXPaymentResponse(headers.get("X-PAYMENT-RESPONSE")!)
                : undefined;
  ...
  private async _getPaymentHeaders(response: Response): Promise<Headers> {
    if (!this._x402Configuration) {
      throw new Error("X402 configuration not set");
    }

    const paymentRequiredResponse = (await response.json()) as {
      x402Version: number;
      accepts: unknown[];
    };

    const { walletClient, maxPaymentValue, paymentRequirementsSelector } =
      this._x402Configuration;

    const parsedPaymentRequirements = paymentRequiredResponse.accepts.map((x) =>
    ...
    if (
      BigInt(selectedPaymentRequirements.maxAmountRequired) > maxPaymentValue
    ) {
  ...
  async send(
    ...
    try {
      const { resumptionToken, onresumptiontoken, headers } = options || {};
      ...
      const allHeaders = await this._commonHeaders();
      if (headers) {
      ...
      allHeaders.set("accept", "application/json, text/event-stream");
      ...
      if (!response.ok) {
        ...
        } else if (response.status === 402 && this._x402Configuration) {
          const paymentHeaders = await this._getPaymentHeaders(response);
