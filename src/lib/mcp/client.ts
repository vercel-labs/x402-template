// forked from https://github.com/vercel/ai/blob/8aadbc90435f2c66abd4ebb764810b0ce69ea127/packages/ai/src/tool/mcp/mcp-client.ts#L395
...
import { MCPClientError } from "./mcp-client-error";
import {
  JSONRPCError,
  JSONRPCNotification,
  JSONRPCRequest,
...
import { MessageExtraInfoWithPayment } from "./http-transport";
...
class DefaultMCPClient implements MCPClient {
  ...
  private async request<T extends ZodType<object>>({
    ...
    return new Promise((resolve, reject) => {
      ...
      this.responseHandlers.set(messageId, (response, extra) => {
        if (signal?.aborted) {
          return reject(
            new MCPClientError({
